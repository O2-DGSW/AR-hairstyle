...
class HairAsset:
    def __init__(self, name, rgba, eye_l, eye_r, ref_skin=None, yaw=None, bank=None,
        ...
        # 같은 GAN 결과에서 오려낸 **얼굴 패치**(이마/눈썹 포함). 기존 머리를
        # 지운 자리를 이걸로 덮는다.
        #
        # 예전에는 보이는 피부의 **단일 평균색**으로 칠했다. 두 가지가 동시에
        # 틀렸다: (1) 평균에 볼/턱/목이 섞여 이마 색과 안 맞고 평평해서 스티커
        # 처럼 보였다. (2) 앞머리에 가려진 눈썹은 파싱이 '머리'로 보므로 지우면
        # 그 자리도 살색이 됐다.
        # GAN 결과에는 그 사람 피부톤과 조명으로 그려진 이마와 눈썹이 이미 들어
        # 있는데 지금까지 머리만 오려내고 버리고 있었다.
        #
        # 같은 눈 앵커를 쓰므로 머리와 똑같은 변환으로 정렬된다.
        self.face = None
