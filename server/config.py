...
class Config:
    ...
    #: GAN 이 잘라먹은 긴 머리를 참고사진의 실제 픽셀로 이어붙일지.
    #:
    #: HairFastGAN 은 FFHQ 정렬 1024 크롭 안에서만 동작하고 그 아래 경계가
    #: 눈에서 약 2.5D 아래다. 가슴까지 오는 머리는 4.6D 까지 내려오므로 길이의
    #: 절반이 모델에 들어가기도 전에 잘린다(실측). 크롭을 넓히는 건 답이 아니다 -
    #: FFHQ quad 배율을 1.8 로만 올려도 얼굴이 뭉개지고 2.4 면 무너진다.
    #: 실측: 이어붙이면 2.58D -> 4.63D 로 원본 길이를 회복한다.
    extend_long_hair: bool = True
